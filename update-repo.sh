...
# 3. Сохраняем текущие изменения (stash)
echo "Сохраняем текущие изменения..."
git stash push -m "Автосохранение перед обновлением репозитория"

# 4. Получаем последние изменения из удаленного репозитория
echo "Получаем последние изменения из удаленного репозитория..."
git pull --rebase

# 5. Восстанавливаем сохраненные изменения
